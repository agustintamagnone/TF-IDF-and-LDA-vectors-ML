...
lda = models.LdaModel(model_bow, num_topics=30, id2word=mapping, random_state=20, passes=2)
lda_v = []
for v in model_bow:
    lda_v.append(lda[v])
...
tfidf_msim = similarities.MatrixSimilarity(lda_v)
...
for doc in docs:
    ...
    for doc_position, doc_score in sims:
        ...
        if _i > 10:
            break
        if docs[doc_position][:12] == "Food & Drink":
            total_goods += 1
        _i += 1

print("Ratio quality: " + str(total_goods / (total_food_drink_docs * 10)))
